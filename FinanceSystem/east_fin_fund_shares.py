...
def handle(a):
    ...
    flag = False
    for i in range(0, len(fund_shares_list)):
        if a[0] == fund_shares_list[i][0]:
            fund_shares_list[i][2] = round(float(fund_shares_list[i][2]) + float(a[4]), 2)
            flag = True
            break
    if not flag:
        new = [a[0], a[1], a[2]]
        fund_shares_list.append(new)
...
def scrape_fund_shares():
    ...
    drop_table_query = "DROP TABLE IF EXISTS fund_shares"
    cursor = db.cursor()
    cursor.execute(drop_table_query)
    db.commit()
    create_table_query = '''CREATE TABLE `fund_shares`  (
    ...
    db.commit()
    try:
        for insert in fund_shares_list:
            sql = "INSERT INTO fund_Shares VALUES ('" + str(insert[0]) + "', '" + str(insert[1]) + "', " + str(
                insert[2]) + ");"
            # 执行sql语句
            cursor.execute(sql)
            # 提交到数据库执行
    ...
    # try:
    df = pd.DataFrame(fund_shares_list, columns=['id', 'name', 'amount'])
